# Hrozby (Threats)

> Zdroj: oficiální pravidla + NotebookLM analýza.
> Poslední aktualizace: 2026-01-31

---

## Typy příšer (Monster Types) — 12 typů

| Typ | Anglicky | Motivace |
|-----|----------|----------|
| Bestie | Beast | Svobodně se pohybovat, ničit a zabíjet |
| Černokněžník | Sorcerer | Uchvátit nadpřirozenou moc |
| Královna | Queen | Posednout a ovládnout |
| Mučitel | Torturer | Způsobit bolest a vyvolat hrůzu |
| Ničitel | Destroyer | Přivodit konec světa |
| Parazit | Parasite | Napadnout, ovládnout a sežrat |
| Pokušitel | Temptation | Svést lidi ke konání zla |
| Popravčí | Executioner | Trestat viníky |
| Požírač | Devourer | Hodovat na lidech |
| Sběratel | Collector | Krást specifický druh věcí |
| Šibal | Trickster | Vnést do všeho chaos |
| Zploditel | Breeder | Přivést zlo na svět nebo ho stvořit |

### Tvorba příšery od nuly

Strážce musí definovat:

1. **Popis** — jak příšera vypadá, jak se chová
2. **Typ a motivace** — výběr z 12 typů (viz tabulka výše)
3. **Nadpřirozené schopnosti** — létání, neviditelnost, ovládání mysli, telekineze...
4. **Útoky** — hodnota zranění (3–5), dosah, štítky
5. **Zbroj** — hodnota 1–2 běžně, vyšší = velká výzva
6. **Výdrž** — minimum 7, běžně 8–12, extrémně 14+
7. **Slabina** — bez ní nelze příšeru trvale zabít

### Kategorie slabin

| Kategorie | Příklady |
|-----------|----------|
| Specifické materiály | Oheň, ryzí železo, stříbro, svěcená voda |
| Specifické metody | Stětí hlavy, dřevěný kolík do srdce |
| Prostředí | Sluneční svit, tekoucí voda, zima |
| Rituály | Magický rituál, řádný pohřeb, exorcismus |

### Tahy příšer (Monster Moves) — 14 tahů

1. Naznačit její přítomnost (Hint at its presence)
2. Odhalit její skutečnou moc (Display its full might)
3. Z čista jasna se objevit (Appear out of nowhere)
4. Vztekle zaútočit vší silou (Attack with fury and contempt)
5. Někoho nebo něčeho se zmocnit (Seize someone or something)
6. Tiše a prohnaně zaútočit (Attack with stealth and calculation)
7. Rozkázat přisluhovačům provést strašné věci (Order minions to terrible deeds)
8. Něco zničit (Destroy something)
9. Uniknout bez ohledu na spoutání (Escape, no matter how well contained)
10. Pronásledovat (Pursue)
11. Vrátit se do svého teritoria (Return to its home ground)
12. Škodolibě se chvástat (Gloat and maybe reveal a secret)
13. Navrátit se po zdánlivém skonu (Return from seeming death)
14. Použít nadpřirozenou schopnost (Use a supernatural power)

---

## Typy přisluhovačů (Minion Types) — 10 typů

| Typ | Anglicky | Motivace |
|-----|----------|----------|
| Hlídač | Guardian | Zahradit cestu nebo něco chránit |
| Kultista | Cultist | Zachránit si kůži za každou cenu |
| Mor | Plague | Zaplavit a zničit |
| Pravá ruka | Right Hand | Stát za příšerou |
| Průzkumník | Scout | Stopovat, sledovat a podat zprávu |
| Renfield | Renfield | Zajistit příšeře přísun obětí |
| Surovec | Brute | Zastrašit a zaútočit |
| Vrah | Assassin | Zabít lovce |
| Zloděj | Thief | Něco pro příšeru ukrást |
| Zrádce | Traitor | Zradit lidi |

### Tahy přisluhovačů — 11 tahů

1. Náhlý záchvat nekontrolovaného násilí
2. Koordinovaně zaútočit
3. Někoho zajmout nebo něco ukrást
4. Prozradit tajemství
5. Odevzdat někoho nebo něco svému pánu
6. Pronásledovat
7. Vyhrožovat ve jménu svého pána
8. Utéct
9. Použít nadpřirozenou schopnost
10. Ukázat záblesk svědomí nebo lidství
11. Malicherným způsobem se vzepřít svému pánu

---

## Typy přihlížejících (Bystander Types) — 9 typů

| Typ | Anglicky | Motivace |
|-----|----------|----------|
| Byrokrat | Official | Podezírat |
| Detektiv | Detective | Zavrhnout rozumné vysvětlení |
| Drbna | Gossip | Šířit klevety |
| Nevinný | Innocent | Zachovat se správně |
| Oběť | Victim | Vystavit se nebezpečí |
| Pomocník | Helper | Přidat se k lovu |
| Skeptik | Skeptic | Odmítat nadpřirozené vysvětlení |
| Svědek | Witness | Poskytnout informace |
| Šťoural | Busybody | Plést se do záležitostí jiných lidí |

### Tahy přihlížejících — 10 tahů

1. Vydat se někam sám
2. Dohadovat se s lovci
3. Překážet
4. Něco prozradit
5. Přiznat se ke svým obavám
6. Ze strachu ztratit hlavu
7. Pokusit se pomoct lovcům
8. Pokusit se ochránit lidi
9. Projevit neschopnost nebo nemohoucnost
10. Hledat pomoc nebo útěchu

### NPC — pravidla

- Každý NPC potřebuje: jméno, stručný popis, typ a motivaci
- Většina NPC vydrží 7 bodů zranění (jako lovci)
- NPC se nevedou tahy lovců, ale tahy hrozby přihlížejících
- Strážce má mít připravený seznam jmen pro náhodná NPC

---

## Typy lokalit (Location Types) — 10 typů

| Typ | Anglicky | Motivace |
|-----|----------|----------|
| Brána pekel | Hellgate | Stvořit zlo |
| Divočina | Wilds | Obsahovat skryté věci |
| Doupě | Den | Ukrývat příšery |
| Knihovna | Hub | Poskytnout informace |
| Křižovatka | Crossroads | Svést lidi a věci dohromady |
| Laboratoř | Lab | Vytvářet prapodivnosti |
| Labyrint | Maze | Zmást a rozdělit |
| Past | Deathtrap | Zranit vetřelce |
| Pevnost | Fortress | Bránit ve vstupu |
| Vězení | Prison | Zadržet a zabránit v úniku |

### Tahy lokalit — 11 tahů

1. Představit hrozbu
2. Něco odhalit
3. Něco skrývat
4. Zabránit v cestě
5. Uvolnit cestu
6. Přetvořit se
7. Někoho polapit do pasti
8. Navést na stopu
9. Představit hlídače
10. Něco tu nefunguje
11. Navodit jedinečný dojem

---

## Fenomén (Phenomenon)

Alternativa k příšeře — nadpřirozená situace bez ústředního nepřítele
(šílená věda, paranormální jevy, biologické nebezpečí).

- **Slabina** = něco, co lovci musí pochopit nebo vyřešit
- V základních pravidlech nemá formální typy (ty přidává Tome of Mysteries)
- Používá tahy lokalit nebo tahy příšer
- Příklady: houba ovládající mysl, smrtící choroba, paranormální úkazy

---

## České folklórní příšery (adaptace v pravidlech)

Sekce „Pár ukázkových hrozeb z místního folklóru" (autor Daniel Vázquez Touriño):

**Příšery:**
| Bytost | Typ |
|--------|-----|
| Vodník | Požírač |
| Hejkal | Mučitel |
| Rýbrcoul / Krakonoš | Popravčí |
| Otesánek | Požírač |

**Přisluhovači:**
| Bytost | Typ |
|--------|-----|
| Bludičky | Zrádce |
| Golem | Hlídač |

**Lokality:**
| Místo | Typ |
|-------|-----|
| Petrovy kameny | Brána pekel |
| Margita a Besná | Doupě |
| Bludný kořen | Labyrint |
